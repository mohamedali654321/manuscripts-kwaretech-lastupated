  ...
  */
            ...
            if (linkedObject?.name) {
              if (!this.formGroup.controls.groupCommunity) {
                ...
                this.formGroup.patchValue({
                  groupName: this.localeService.getStringByLocale(activeGroup.name),
                  groupCommunity: linkedObject?.name ?? '',
                  groupDescription: this.localeService.getStringByLocale(activeGroup.firstMetadataValue('dc.description')),
                });
              }
            } else {
              this.formModel = [
      ...
      (group: Group) => {
        const values = {
          ...
          metadata: {
            'dc.description': [
              {
                value: this.localeService.getStringByLocale(this.groupDescription.value),
              },
            ],
          },
        };
  ...
  editGroup(group: Group) {
    ...
    this.groupDataService.patch(group, operations).pipe(
      ...
      if (rd.hasSucceeded) {
        this.notificationsService.success(this.translateService.get(this.messagePrefix + '.notification.edited.success', { name:this.localeService.getStringByLocale(this.dsoNameService.getName(rd.payload))  }));
        this.submitForm.emit(rd.payload);
      } else {
        this.notificationsService.error(this.translateService.get(this.messagePrefix + '.notification.edited.failure', { name: this.localeService.getStringByLocale(this.dsoNameService.getName(group)) }));
        this.cancelForm.emit();
      }
    });
  }
          ...
          if (hasValue(group.id)) {
            this.groupDataService.delete(group.id).pipe(getFirstCompletedRemoteData())
              .subscribe((rd: RemoteData<NoContent>) => {
                if (rd.hasSucceeded) {
                  this.notificationsService.success(this.translateService.get(this.messagePrefix + '.notification.deleted.success', { name:this.localeService.getStringByLocale( this.dsoNameService.getName(group) )}));
                  this.onCancel();
                } else {
                  this.notificationsService.error(
                    this.translateService.get(this.messagePrefix + '.notification.deleted.failure.title', { name: this.localeService.getStringByLocale(this.dsoNameService.getName(group)) }),
